import numpy as np
from core.data_io import coerce_num        # ABSOLUTE IMPORT
...
def enrich_insights(insights):
    ...
    fixed_unalloc = (
        coerce_num(ins.get("fixed_costs", 0)).fillna(0)
        + coerce_num(ins.get("unalloc", 0)).fillna(0)
        + coerce_num(ins.get("mgmt_fees", 0)).fillna(0)
    )
